...
# print('N', rec.total_samples)
  ...
  #  print (i, lbl)
    if 'V-node' in lbl:
      key = ('V:' + lbl[0:6] + ':' + lbl[8:14]).rstrip()
      chan[key] = np.array (rec.analog[i])
    elif 'I-branch' in lbl:
      if lbl[0:4] == 'TACS':
        key = ('T:' + lbl[8:14]).rstrip()
      elif lbl[0:6] == 'MODELS':
        key = ('M:' + lbl[8:14]).rstrip()
      else:
        key = ('I:' + lbl[0:6] + ':' + lbl[8:14]).rstrip()
      chan[key] = np.array (rec.analog[i])
  return chan

def load_atp_comtrade_channels (atp_base, filtered=None, k=1000, method='slice', pv1=False, lab1=False, gfm8=False):
  rec = Comtrade ()
  ...
  if pv1:
    ...
    # channels = find_channels (rec) # what was this for?
    channels = find_all_channels (rec)
