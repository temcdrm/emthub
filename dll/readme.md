...
3. Build the _wrapper_ project, as the following four examples depend on it.
4. Build and test the _SEXS_ example, which is a self-contained static excitation system DLL.
5. Build and test the _GFM_GFL_IBR2_ example, which is a self-contained inverter-based resource (IBR) controller from EPRI.
...
7. Build and test the _PPC_ example, which is a renewable plant controlller model (WECC REPCA) compiled with OpenModelica (a required download for this example).
