...
def find_base_voltage (dict, bus_id):
  for table_name in ['ACLineSegment', 'SeriesCompensator']:
    d = dict[table_name]
    for key, row in d['vals'].items():
      if row['from'] == bus_id or row['to'] == bus_id:
        bv = row['bv']
        return dict['BaseVoltage']['vals'][bv]['nominalVoltage']
  return -1000.0
...
def convert_one_atp_model (d, fpath, case):
  ...
  *atpmap* file that maps CIM bus (ConnectivityNode) names to ATP bus numbers.
  ...
  atp_buses = {}  # find the ATP bus number from the CIM ConnectivityNode mRID
  cim_names = {}  # find the CIM ConnectivityNode name from the CIM ConnectivityNode mRID
  bus_kv = {}     # nominal voltage (line-to-line kV) from the CIM ConnectivityNode mRID
  cim_mrids = {}  # find the CIM ConnectivityNode mRID from the ATP bus number
  bNumeric = True
  for key in d['ConnectivityNode']['vals']:
    if not key.isdigit():
      bNumeric = False
      break
  if bNumeric:
    ordered_buses = dict(sorted(d['ConnectivityNode']['vals'].items(), key=lambda x:int(x[0])))
    print ('numeric ordered buses')
  else:
    ordered_buses = d['ConnectivityNode']['vals']
  ...
  for key, data in ordered_buses.items():
    ...
    atp_buses[key] = bus
    cim_mrids[bus] = key
    cim_names[key] = data['name']
    if cim_names[key] == swingbus:
      swingbus_key = key
    bus_kv[key] = 0.001 * find_base_voltage (d, key)
    idx += 1
  print ('Mapping {:d} buses to {:s}.atpmap'.format (idx-1, fname))
  fp = open (fpath + fname + '.atpmap', mode='w')
  print ('ATP Bus  CIM Key                              CIM Name              Bus kV', file=fp)
  for bus, mrid in cim_mrids.items():
    print ('{:6s}   {:36s} {:20s} {:7.3f}'.format (bus, mrid, cim_names[mrid], bus_kv[mrid]), file=fp)
  print ('\nCIM Key                              CIM Name             ATP Bus   Bus kV', file=fp)
  for mrid, bus in atp_buses.items():
    print ('{:36s} {:20s} {:6s}   {:7.3f}'.format (mrid, cim_names[mrid], bus, bus_kv[mrid]), file=fp)
  fp.close()
  ...
  bus = atp_buses[swingbus_key]
  kv = bus_kv[swingbus_key]
  x1pu_swing = 0.050
  r1pu_swing = 0.005
  mva_swing = 1000.0
  p_swing = 0.0
...
#   xfbusnum += 1
  ...
  for key, row in d['ACLineSegment']['vals'].items():
    bus1 = AtpBus(atp_buses[row['from']])
    bus2 = AtpBus(atp_buses[row['to']])
    rs = (row['r0']+2.0*row['r1']) / 3.0
    rm = (row['r0']-row['r1']) / 3.0
    xs = (row['x0']+2.0*row['x1']) / 3.0
    xm = (row['x0']-row['x1']) / 3.0
    cs = 1.0e6 * (row['b0ch']+2.0*row['b1ch']) / 3.0 / OMEGA
    cm = 1.0e6 * (row['b0ch']-row['b1ch']) / 3.0 / OMEGA
    if rs < 0.0 or rm < 0.0:
    ...
    if cs > 0.0:
      ...
      if min(tau0, tau1) >= MIN_BERGERON_TAU:
        bergeron = True
    print ('C =============================================================================', file=ap)
    print ('C line {:s} from {:s} to {:s}'.format (row['name'], cim_names[row['from']], cim_names[row['to']]), file=ap)
  ...
  for key, row in d['SeriesCompensator']['vals'].items():
    bus1 = AtpBus(atp_buses[row['from']])
    bus2 = AtpBus(atp_buses[row['to']])
    if row['x1'] > 0.0:
      print ('C =============================================================================', file=ap)
      print ('C series reactor {:s} from {:s} to {:s}'.format (row['name'], cim_names[row['from']], cim_names[row['to']]), file=ap)
      print ('C < n1 >< n2 ><ref1><ref2>< R  >< X  >< C  >', file=ap)
      print ('51{:5s}A{:5s}A            {:s}{:s}'.format (bus1, bus2, AtpFit6 (row['r0']), AtpFit6 (row['x0'])), file=ap)
      print ('52{:5s}B{:5s}B            {:s}{:s}'.format (bus1, bus2, AtpFit6 (row['r1']), AtpFit6 (row['x1'])), file=ap)
      print ('53{:5s}C{:5s}C'.format(bus1, bus2), file=ap)
    else:
      cuf = -1.0e6 / row['x1'] / OMEGA
      print ('C =============================================================================', file=ap)
      print ('C series capacitor {:s} from {:s} to {:s}'.format (row['name'], cim_names[row['from']], cim_names[row['to']]), file=ap)
      print ('C < n1 >< n2 ><ref1><ref2>< R  >< X  >< C  >', file=ap)
  ...
  for key, row in d['EMTCompShunt']['vals'].items():
    ...
    if kvar > 0.0:
      ...
      print ('C =============================================================================', file=ap)
      print ('C capacitor {:s} at {:s} is {:.2f} kVAR'.format (row['name'], cim_names[row['bus']], kvar), file=ap)
      print ('C < n 1>< n 2><ref1><ref2><       R      ><      X       ><      C       ><   >', file=ap)
      print ('$VINTAGE,1', file=ap)
      ...
      bus = atp_buses[row['bus']]
      print ('C =============================================================================', file=ap)
      print ('C reactor {:s} at {:s} is {:.2f} kVAR'.format (row['name'], cim_names[row['bus']], -kvar), file=ap)
      print ('C < n 1>< n 2><ref1><ref2><       R      ><      X       ><      C       ><   >', file=ap)
      print ('$VINTAGE,1', file=ap)
      for ph in GetAtpPhaseList ('ABC'):
  ...
  for key, row in d['EMTLoad']['vals'].items():
    ...
    if rload < 0.0:
      ...
      sPF = AtpFit6 (pfang)
      print ('C =============================================================================', file=ap)
      print ('C DER {:s} at {:s} is {:.2f} MVA producing {:.2f} MW'.format (row['name'], cim_names[row['bus']], 1e-6*sbase, 1e-6*wtotal), file=ap)
      ...
      print ('C =============================================================================', file=ap)
      print ('C load {:s} at {:s} is {:.3f} + j{:.3f} kVA'.format (row['name'], cim_names[row['bus']], kw, kvar), file=ap)
      print ('C < n 1>< n 2><ref1><ref2><       R      ><      X       ><      C       ><   >', file=ap)
      print ('$VINTAGE,1', file=ap)
      for ph in phases:
  ...
  for key, row in d['EMTSolar']['vals'].items():
    ...
    sPF = AtpFit6 (pfang)
    print ('C =============================================================================', file=ap)
    print ('C solar {:s} at {:s} is {:.3f} MVA producing {:.3f} MW'.format (row['name'], cim_names[row['bus']], sbase*1e-6, wtotal*1e-6), file=ap)
  ...
  for key, row in d['EMTWind']['vals'].items():
    ...
    WND_COUNT += 1
    print ('C =============================================================================', file=ap)
    print ('C wind {:s} at {:s} is {:.3f} MVA producing {:.3f} MW'.format (row['name'], cim_names[row['bus']], sbase*1e-6, wtotal*1e-6), file=ap)
    AppendSolar (bus, vbase, sbase, ibase=sbase/vbase, ppu=wtotal/sbase, qpu=row['q']/sbase, vpu=1.0, ap=ap, ibr_count=PV_COUNT+WND_COUNT)
  ...
  if len(machines) > 0: # ATP requires these come after all other sources
    ...
    for key, row in machines.items():
      ...
      if row['bus'] == swingbus:
        print ('C =============================================================================', file=ap)
        print ('C SyncMachine {:s} at {:s} is {:.2f} MVA, part of Swing Bus'.format (row['name'], cim_names[row['bus']], rmva), file=ap)
      else:
        print ('C =============================================================================', file=ap)
        print ('C SyncMachine {:s} at {:s} is {:.2f} MVA, {:.2f} MW, {:.2f} MVAR'.format (row['name'], cim_names[row['bus']], rmva, mw, mvar), file=ap)
