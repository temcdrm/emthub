...
def build_system_graph (d):
  ...
  for key, data in d['EMTLoad']['vals'].items():
    buses[data['ConnectivityNode1_mRID']]['has_load'] = True
  for key, data in d['EMTSyncMachine']['vals'].items():
    buses[data['ConnectivityNode1_mRID']]['has_gen'] = True
  for key, data in d['EMTSolar']['vals'].items():
    buses[data['ConnectivityNode1_mRID']]['has_solar'] = True
  for key, data in d['EMTWind']['vals'].items():
    buses[data['ConnectivityNode1_mRID']]['has_wind'] = True
  G = nx.Graph()
  ...
  for key, data in d['EMTPowerXfmrWinding']['vals'].items():
    ...
    if pname not in xfmrs:
      xfmrs[pname] = {busnum:data['ConnectivityNode1_mRID']}
  ...
  for key, data in d['EMTLine']['vals'].items():
    ...
    G.add_edge(data['ConnectivityNode1_mRID'],data['ConnectivityNode2_mRID'],eclass='line',eid=key,
